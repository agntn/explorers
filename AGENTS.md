...
- `Transaction.to` is `null` whenever the explorer names no recipient, not only on a contract creation: Solscan, Helius and Blockberry never name one. Etherscan and Blockscout put the deployed address in `createdContract`, and the CLI, Pi and OMP renderers print a recipient or a created contract only when the record has one
