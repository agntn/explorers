...
`dcrdata` reads Decred balances, transactions and blocks without an API key. Balance calls use the [Insight address endpoint](https://github.com/decred/dcrdata/blob/master/docs/Insight_API_documentation.md#addr) with `noTxList=1`, not the full transaction list. Amounts come from integer atom fields (8 decimals). `balance`, `funded` and `spent` stay confirmed. Pending activity shows up as the signed `unconfirmed` delta, the same field Mempool and Blockstream fill. Insight supplies no snapshot height or hash, so both stay `null`. These are indexed balances, not a guarantee that every output is mature or spendable.
