...
Library modules evaluate to declarations only. A derived value, a `new Set()`, a prebuilt map, a decoder, waits for its first use. `sideEffects` in `package.json` names the CLI bundle and nothing else, and that's what lets a consumer import one provider without paying for thirteen. A custom provider that keeps to the same rule stays tree-shakable in the bundle that carries it.
