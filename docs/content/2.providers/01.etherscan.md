...
- Some reads sit behind a paid plan on some chains, and you find out at runtime. The gas oracle on BNB Chain is one. That's a `PlanRestrictedError`, and the fallback pass tries Blockscout if it serves the chain, which for BNB Chain it doesn't.
