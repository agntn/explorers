/**
 * Get current gas prices
 */
import { defineCommand } from 'citty'
import consola from 'consola'
import { resolveProvider, PROVIDER_DEFAULT_CHAIN } from '../core/resolve.js'
import { create } from '../core/registry.js'
import { normalizeChain } from '../core/types.js'

export default defineCommand({
  meta: {
    name: 'gas',
    description: 'Get current gas prices',
  },
  args: {
    chain: {
      type: 'string',
      alias: 'c',
      description: 'Chain',
      default: 'eth',
    },
    provider: {
      type: 'string',
      alias: 'p',
      description: 'Provider',
    },
  },
  async run({ args }) {
    const providerName = resolveProvider(args.provider as string | undefined)
    const provider = create(providerName)
    const defaultChain = PROVIDER_DEFAULT_CHAIN[providerName] ?? args.chain
    const chain = normalizeChain(defaultChain as string)

    const caps = provider.capabilities()
    if (!caps.gasData) {
      consola.error(`Provider "${providerName}" does not support gas data`)
      process.exit(1)
    }

    try {
      const gas = await provider.getGasData!(chain)
      consola.log(`[${providerName}] Gas prices on ${gas.chain}`)
      if (gas.safeGasPrice) consola.log(`  Safe/Low: ${gas.safeGasPrice} gwei`)
      if (gas.proposedGasPrice) consola.log(`  Average:  ${gas.proposedGasPrice} gwei`)
      if (gas.fastGasPrice) consola.log(`  Fast:     ${gas.fastGasPrice} gwei`)
      if (gas.baseFee) consola.log(`  Base fee: ${gas.baseFee} gwei`)
      if (gas.priorityFee) consola.log(`  Priority: ${gas.priorityFee} gwei`)
    }
    catch (error) {
      consola.error(`Error: ${error instanceof Error ? error.message : error}`)
      process.exit(1)
    }
  },
})
