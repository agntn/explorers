...
async function selectedProvider(
  ...
  const capability = operation ? OPERATION_CAPABILITIES[operation] : undefined;
  const name = resolveProvider(providerName, requestedChain, capability);
...
export function createMcpServer(): McpServer {
  ...
  server.registerTool(
    ...
    async ({ address, chain, provider }) => {
      const selected = await selectedProvider(provider, chain, "getBalance");
