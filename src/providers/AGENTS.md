...
- Declare chain, capability, and endpoint metadata in `index.ts`; keep capabilities aligned with the instance getter and add each provider file to `pack.entry` in `vite.config.ts`.
