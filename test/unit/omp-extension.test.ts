...
`,
  ...
  it("aborts the explorer request when the host cancels the tool", async () => {
    ...
    const execution = requireTool(registerExtensionTools().tools, "explorers_balance")
      .execute(
        "test",
  ...
  it("documents environment-based provider selection accurately", () => {
    ...
    expect(tool.description).toContain("other addresses read Ethereum unless a provider is named");
  ...
  it("reads a legacy address as Bitcoin SV when the call names WhatsOnChain", async () => {
    ...
    const balance = requireTool(registerExtensionTools().tools, "explorers_balance");
    const result = parseToolResult(
  ...
  it("reads Stellar stroops and trustlines through automatic provider selection", async () => {
    ...
    const tokens = requireTool(registerExtensionTools().tools, "explorers_tokens");
